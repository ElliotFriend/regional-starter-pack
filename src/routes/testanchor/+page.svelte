...
<div class="mx-auto max-w-4xl space-y-6">
	<div>
		...
		<p class="mt-2 text-gray-600">
			Interactive testing with <code class="rounded bg-gray-100 px-1">testanchor.stellar.org</code>
	...
	{#if error}
		<div class="rounded-lg border border-red-200 bg-red-50 p-4 text-red-700">
			<div class="flex items-start justify-between">
				<span>{error}</span>
				<button onclick={() => (error = null)} class="ml-4 text-red-500 hover:text-red-700" aria-label="Dismiss error">
					...
					<div class="rounded-lg bg-gray-50 p-4">
						...
						<div class="flex flex-wrap gap-2">
							{#each tomlInfo.currencies as currency (currency.code)}
								<span class="rounded-full bg-violet-100 px-3 py-1 text-sm text-violet-800">
									{currency.code}
								</span>
							{/each}
						</div>
	...
	<section class="rounded-lg border border-gray-200 bg-white p-6">
		...
		{:else if authenticated}
			<div class="flex items-center gap-2 text-green-600">
				...
				Authenticated as {walletStore.publicKey?.slice(0, 8)}...{walletStore.publicKey?.slice(-8)}
			</div>
		{:else}
			<button
	...
	<section class="rounded-lg border border-gray-200 bg-white p-6">
		...
		{:else}
			...
			<div class="mb-6 space-y-4">
				...
				<fieldset>
					<legend class="mb-2 block text-sm font-medium text-gray-700">Transfer Type</legend>
				...
				<div class="grid gap-4 sm:grid-cols-2">
					<!-- Asset Selection -->
					<div>
						<label for="asset" class="mb-2 block text-sm font-medium text-gray-700">Asset</label>
						<select
							id="asset"
						...
						<div class="mt-4 rounded-lg bg-white p-3">
							...
							{#if sep6Result.id}
								<p class="text-sm text-gray-700">
									...
									<code class="ml-1 rounded bg-gray-100 px-1">{sep6Result.id}</code>
								</p>
							{/if}
							{#if sep6Result.eta}
								<p class="text-sm text-gray-700">
							...
							{/if}
							<details class="mt-2">
								<summary class="cursor-pointer text-xs text-gray-500">Full response</summary>
								<pre class="mt-2 overflow-x-auto text-xs">{JSON.stringify(sep6Result, null, 2)}</pre>
							</details>
						</div>
					{/if}
				</div>
				...
				<div class="mt-4 space-y-4">
					{#if sep6Info}
						<div class="rounded-lg bg-gray-50 p-4">
							...
							<pre class="overflow-x-auto text-xs">{JSON.stringify(sep6Info, null, 2)}</pre>
						</div>
					{/if}
					{#if sep24Info}
						<div class="rounded-lg bg-gray-50 p-4">
							<h3 class="mb-2 font-medium text-gray-900">SEP-24 Info:</h3>
							<pre class="overflow-x-auto text-xs">{JSON.stringify(sep24Info, null, 2)}</pre>
						</div>
					{/if}
					{#if sep38Info}
						<div class="rounded-lg bg-gray-50 p-4">
							<h3 class="mb-2 font-medium text-gray-900">SEP-38 Info:</h3>
							<pre class="overflow-x-auto text-xs">{JSON.stringify(sep38Info, null, 2)}</pre>
						</div>
					{/if}
				</div>
			{/if}
	...
	<section class="rounded-lg border border-gray-200 bg-gray-50 p-6">
		...
		<p class="text-gray-600">
			The Stellar test anchor at <code class="rounded bg-gray-200 px-1">testanchor.stellar.org</code
			>
			is a reference implementation maintained by the Stellar Development Foundation. It supports all
			standard SEPs and is designed for testing anchor integrations without using real assets.
		</p>
