...
There is no on-ramp fiat-received simulation API, so only Khipu can push an order past `WAITING` in the sandbox: paying `1234` / `123456` on the Khipu test page confirms the fiat and the order advances `WAITING → EXECUTING → IN_PROGRESS → DELIVERED`, landing canonical USDC on Stellar testnet at the destination address (matching the quoted amount). WIREAR and QRI orders stay in `WAITING` (no sandbox pay page). Verified end-to-end 2026-07-08 (order `31a3e3ab-…`, delivery tx `6620885625…`, asset `USDC`/issuer `GBBD47…FLA5` = the configured `PUBLIC_USDC_ISSUER`) after Koywe enabled USDC-on-Stellar in the sandbox — this supersedes the earlier `EXECUTING`/`PENDING` retry-loop, which was a Koywe-side gap before they enabled the Stellar leg. `getOrder` surfaces `dates`, `txHash`, `statusDetails`, and `isDeliveryExpired` so the on-ramp page can show where an order stalls.

A note on the issuer: Koywe returns no Stellar issuer for `USDC Stellar` (see `token-currencies`), and we inject `PUBLIC_USDC_ISSUER`. The 2026-07-08 delivery paid from that same configured issuer (`GBBD47…FLA5`), confirming the injected-issuer approach settles correctly in the sandbox.
...
On-ramp is now verified end-to-end (see Sandbox quirks). Off-ramp remains blocked Koywe-side. Remaining work, none of which is a code-shape bug on our side:

1. **[DONE] On-ramp crypto delivery** — Koywe enabled USDC-on-Stellar in the sandbox; a Khipu on-ramp now settles canonical USDC on Stellar testnet end-to-end (verified 2026-07-08). No longer a blocker.
2. **[Koywe team] Off-ramp bank-account ownership validation** — `POST /rest/bank-accounts` still rejects Koywe's own documented DNI↔CVU test pairs with a `400` ("Failed to validate if bank account … belongs to …"), re-confirmed 2026-07-08 on a fresh account KYC'd under whitelisted DNI `14013056`. Blocks the entire off-ramp. Needs Koywe to fix the sandbox validation backend (or tell us what the request is missing).
...
5. **[us] Confirm off-ramp to `DELIVERED`** — on-ramp is confirmed; off-ramp still can't be verified end-to-end until blocker #2 is resolved.
