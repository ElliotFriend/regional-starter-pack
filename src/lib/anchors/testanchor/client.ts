...
export class TestAnchorClient {
	...
	async getSep24Transaction(transactionId: string): Promise<Sep24Transaction> {
		...
		return sep24.getTransaction(transferServer, this.requireAuth(), transactionId, this.fetchFn);
	}
	...
	async createSep31Transaction(
		...
		return sep31.postTransaction(directPaymentServer, this.requireAuth(), request, this.fetchFn);
	}
	...
	async getSep31Transaction(transactionId: string): Promise<Sep31Transaction> {
		...
		return sep31.getTransaction(directPaymentServer, this.requireAuth(), transactionId, this.fetchFn);
	}
